...
no_gen<-data.frame(mar_smelt_rare %>% group_by() %>% mutate(total_abund=Abundance/sum(Abundance)) %>%
  filter(sub_type=="northern") %>% filter(Genus=="g__") %>%
  filter(rel_abund>0.01) %>% distinct(OTU, .keep_all=TRUE) %>%
    arrange(Class,Order,Family))

data.frame(mar_smelt_rare %>% group_by() %>% mutate(total_abund=Abundance/sum(Abundance)) %>%
  filter(sub_type=="northern") %>% filter(Genus=="g__") %>% group_by(sampleID) %>%
  summarise(sam_sum=sum(rel_abund)) %>% arrange(desc(sam_sum)))

data.frame(mar_smelt_rare %>% group_by() %>% mutate(total_abund=Abundance/sum(Abundance)) %>%
  filter(sub_type=="northern") %>% filter(Family=="f__") %>% group_by(sampleID) %>%
  summarise(sam_sum=sum(rel_abund)) %>% arrange(desc(sam_sum)))
```
